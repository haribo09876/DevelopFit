...
//				저장소에 실제로 파일이 만들어 짐
				...
				fileInfoMap.put("original_file_name", originalFileName);		//	원래 파일 이름
				fileInfoMap.put("stored_file_name", storedFileName);		//	겹치지 않는 파일 이름
				fileInfoMap.put("file_size", multipartFile.getSize());

				fileList.add(fileInfoMap);		//	이미지가 다수인 경우 사용
			}
		}
		return fileList;
	}
